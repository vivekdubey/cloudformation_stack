...
class CFStackService
  ...
  def deploy(disable_rollback=false,timeout=3600,tags=[])
    if exists?
      ...
      exception_handler { update(timeout.to_i,tags) }
    else
      Log.info "Creating cloudformation stack...\nStack Name: #{stack_name}"
      exception_handler { create(disable_rollback,timeout.to_i,tags) }
    end
  end
  ...
  def create(disable_rollback,timeout,tags)
    output = @cf_client.create_stack(disable_rollback,timeout,tags)
  ...
  def update(timeout,tags)
    output = @cf_client.update_stack(timeout,tags)
